 ...
 * distancias Manhattan sin bloqueos, mantenimiento ni turnos; un positivo solo acredita esta politica simplificada. Un negativo tampoco demuestra
 * imposibilidad en el modelo completo, que admite otros almacenes y posiciones iniciales.</p>
 */
public final class PruebaFactibilidadIndependiente {
    ...
    public static Resultado evaluar(RepositorioDatos.DatosEscenario datos,
        ...
        // Las velocidades se leen de parametros sin modificar la flota del llamador.
    ...
    public static Resultado evaluarPedidos(List<Pedido> pedidos, List<UnidadTransporte> unidades,
        ...
        int capacidadCentralInsuficiente = 0;
        ...
        for (Pedido pedido : ordenados) {
            ...
            if (capacidadOptimista < pedido.cantidad()) {
                capacidadCentralInsuficiente++;
                diagnosticos.add(new Diagnostico(pedido.id(), "CAPACIDAD_CENTRAL_INSUFICIENTE",
            ...
            while (restante > 0) {
                ...
                elegida.disponibleDesde = mejorLlegada + parametros.minutosAcondicionamiento() + vuelta;
            }
        }
        return new Resultado(capacidadCentralInsuficiente == 0 && noProgramables == 0,
                pedidos.size(), capacidadCentralInsuficiente, noProgramables, diagnosticos);
    }
    ...
    private static int capacidadOptimista(Pedido pedido, List<EstadoUnidad> unidades, int central,
        ...
        for (EstadoUnidad unidad : unidades) {
            ...
            int duracion = ida + parametros.minutosAcondicionamiento() + vuelta;
            long ventana = pedido.minutoLimite() - pedido.minutoRegistro();
            if (duracion > 0 && ventana >= ida) {
                long viajes = 1L + Math.max(0L, ventana - ida) / duracion;
                total = Math.min(Integer.MAX_VALUE, total + Math.min(Integer.MAX_VALUE,
    ...
    public record Resultado(boolean pasa, int pedidos, int capacidadCentralInsuficiente,
